...
namespace WindowResizer.CLI.Commands
{
    ...
    public class InteractiveOption : Option<bool>
    {
        public InteractiveOption() : base(
            ...
            description: "Interactively choose a visible top-level window/application before resizing. Use arrow keys, letter keys, and Enter, or Esc to quit. Shows PID and marks the frontmost listed window for each process name as Top. The selected row uses a dark inverted highlight background.")
        {
            IsRequired = false;
            AllowMultipleArgumentsPerToken = false;
        }
